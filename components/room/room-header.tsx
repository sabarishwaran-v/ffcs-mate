...
import { Users, LogOut, Copy, Check, Shield, Eye, ShieldAlert, Inbox } from "lucide-react";
...
import { doc, onSnapshot, updateDoc, deleteField, arrayRemove, query, collection, where, arrayUnion, serverTimestamp, setDoc } from "firebase/firestore";
...
export function RoomHeader({ roomId }: { roomId: string }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, "rooms", roomId), (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (prevHostRef.current && prevHostRef.current !== newHostId && newMembers[newHostId]) {
        ...
        if (prevMembersRef.current) {
          ...
          newKeys.forEach(key => {
            ...
            if (!prevKeys.includes(key) && newMember.regNo) {
              if (key !== user?.uid) toast.success(`${newMember.regNo} joined the room!`);
            } else if (prevMember && newMember && prevMember.role !== newMember.role) {
              if (key !== user?.uid) {
                toast.info(`${newMember.regNo} was moved to ${newMember.role === "spectator" ? "Spectator Mode" : "Editor"}`);
              } else {
                toast.info(`You were moved to ${newMember.role === "spectator" ? "Spectator Mode" : "Editor"}`);
              }
            }
          });
          ...
          prevKeys.forEach(key => {
            if (!newKeys.includes(key) && prevMembersRef.current![key].regNo) {
              if (key !== user?.uid) toast.error(`${prevMembersRef.current![key].regNo} left the room.`);
            }
          });
        }
  ...
  const handleAcceptRequest = async (inv: any) => {
    try {
      if (Object.keys(members).length >= 8) {
        toast.error("Room is full!", { description: "You cannot have more than 8 members in a room." });
        return;
      }
  ...
  const copyRoomCode = () => {
    ...
    setCopied(true);
    toast.success("Room Code Copied", { description: "Share this code with your friends!" });
  ...
  const toggleSpectatorMode = async (memberUid: string, currentRole: string) => {
    ...
    try {
      ...
      toast.success(newRole === "spectator" ? "User moved to Spectator Mode" : "Edit access restored");
    } catch (e) {
  ...
  Object.entries(members).forEach(([uid, m]: [string, any]) => {
    const lastSeenMs = m.lastSeen ? (m.lastSeen.seconds ? m.lastSeen.seconds * 1000 : m.lastSeen.toMillis ? m.lastSeen.toMillis() : now) : now;
    ...
    <div className="mb-8 rounded-xl border bg-card p-4 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4 relative z-20">
      ...
      <div className="flex flex-wrap items-center justify-center gap-4">
        <Popover>
          <PopoverTrigger asChild>
            <button className="flex flex-col items-start gap-1 p-2 rounded-lg hover:bg-muted/50 transition-colors border border-transparent hover:border-border text-left">
              ...
              <div className="flex -space-x-2">
                {Object.entries(members).map(([uid, m]: [string, any], index) => {
                  const colorClass = getAvatarColor(uid);
                  const isOnline = !!activeMembersObj[uid];
                  return (
                    <div 
                      key={uid}
                      className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shadow-sm ring-2 ring-background z-[${10 - index}] ${colorClass} relative`}
          ...
          <PopoverContent align="end" className="w-72 p-0 border border-border shadow-xl">
            ...
            <div className="max-h-[300px] overflow-y-auto p-2 flex flex-col gap-1">
              {Object.entries(members).map(([uid, m]: [string, any]) => {
                const isOnline = !!activeMembersObj[uid];
                return (
                <div key={uid} className="flex items-center justify-between p-2 rounded-md hover:bg-muted/50 transition-colors">
                  <div className="flex items-center gap-2">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shadow-sm ${getAvatarColor(uid)} relative`}>
                      {getInitials(m.name)}
                      {isOnline && (
                        <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-500 border-2 border-background rounded-full"></span>
                      )}
                    ...
                    <div className="flex flex-col">
                      <span className="text-sm font-medium flex items-center gap-1.5">
                        {m.name}
                        {uid === hostId && <Shield className="w-3 h-3 text-yellow-500" />}
                        {uid === user?.uid && <span className="text-[10px] text-muted-foreground">(You)</span>}
                      </span>
                      <span className="text-[10px] text-muted-foreground uppercase">{m.role || "editor"}</span>
                    </div>
                  </div>
                  {isHost && uid !== hostId && (
                    <Button 
                      ...
                      className={`h-7 text-xs px-2 ${m.role === "spectator" ? "text-green-500 hover:text-green-600 hover:bg-green-500/10" : "text-red-500 hover:text-red-600 hover:bg-red-500/10"}`}
                    >
                      {m.role === "spectator" ? "Restore Access" : "Revoke Access"}
                    </Button>
                  )}
        ...
        {isHost && requests.length > 0 && (
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className="relative border-purple-500/30 text-purple-600 hover:bg-purple-500/10 hover:text-purple-700 h-9">
            ...
            <PopoverContent className="w-80 p-0" align="end">
              ...
              <div className="max-h-[300px] overflow-y-auto p-2 flex flex-col gap-2">
                {requests.map(req => (
                  <div key={req.id} className="border p-3 rounded-lg flex flex-col gap-2 bg-card shadow-sm">
                    <div>
                      <p className="font-medium text-sm">{req.fromName}</p>
                      <p className="text-xs text-muted-foreground">{req.fromRegNo}</p>
                    </div>
                    <div className="flex gap-2 w-full">
                      <Button size="sm" variant="outline" className="h-7 text-xs flex-1 border-red-500/30 text-red-500 hover:bg-red-500/10" onClick={() => handleRejectRequest(req)}>Decline</Button>
                      <Button size="sm" className="h-7 text-xs flex-1 bg-purple-600 hover:bg-purple-700 text-white" onClick={() => handleAcceptRequest(req)}>Approve</Button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <Button variant="ghost" size="icon" onClick={leaveRoom} className="text-red-500 hover:text-red-600 hover:bg-red-500/10 sm:hidden">
          <LogOut className="w-4 h-4" />
        </Button>
        <Button variant="outline" size="sm" onClick={leaveRoom} className="text-red-500 hover:text-red-600 hover:bg-red-500/10 border-red-500/20 hidden sm:flex">
