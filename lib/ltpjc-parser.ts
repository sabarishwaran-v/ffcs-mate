...
export function parseLTPJC(course: Course): LTPJC {
  ...
  if (
    course.theorySlots &&
    course.theorySlots.length > 0 &&
    course.theorySlots[0] !== "NILL"
  ) {
...
export function getCreditsFromSlotString(
  slotString: string,
  course?: Course
): number {
  ...
  if (
    course &&
    (course.type === "PJT" || course.hasProjectComponent) &&
    theoryTokens > 0
  ) {
    C += 1;
  }
