...
window.addCourseToCourseList = (courseData) => {
    var isLab = courseData.slots[0][0] == 'L' ? 'true' : 'false';
    var primarySlot = courseData.slots[0];
    var $courseListItem = $(
        `<tr
            data-course="course${courseData.courseId}"
            data-is-project="${courseData.isProject}"
            data-is-lab="${isLab}"
            data-course-code="${courseData.courseCode}"
            data-slot-raw="${primarySlot}"
        >
    ...
    /*
        If a column header has been manually clicked by the user to sort,
    ...
    if (sortedColumn != undefined) {
        ...
        // If NO column is manually sorted, append it and use Custom Sort!
        $('#course-list tbody').append($courseListItem);
        
        // Custom Sort: Theory slot first, then its respective Lab immediately after.
        var rows = $('#course-list tbody tr').get();
        var theoryMap = {}; // Maps courseCode -> theorySlot

        // First pass: Find all Theory slots for each course code
        rows.forEach(row => {
            var $row = $(row);
            if ($row.attr('data-is-lab') !== 'true') {
                theoryMap[$row.attr('data-course-code')] = $row.attr('data-slot-raw');
            }
        });
        ...
        rows.sort((a, b) => {
            const getSortKey = (row) => {
                var $row = $(row);
                var isLab = $row.attr('data-is-lab') === 'true';
                var courseCode = $row.attr('data-course-code');
                var slot = $row.attr('data-slot-raw');
                
                if (isLab) {
                    var parentTheorySlot = theoryMap[courseCode];
                    if (parentTheorySlot) {
                        return parentTheorySlot + '-Z_LAB_' + slot; // Theory first, then Lab
                    } else {
                        return 'Z_' + slot; // Lab with no theory goes at the end
                    }
                }
                return slot + '-A_THEORY'; 
            };
            
            var keyA = getSortKey(a);
            var keyB = getSortKey(b);
